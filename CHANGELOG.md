...
- Inline XML elements translated to Markdown: `<c>`, `<code>`, `<see>`, `<a>`, `<paramref>`, `<typeparamref>`, `<para>`, `<br>`, `<b>`, `<i>`.
- `<list>` as a Markdown list: bulleted, or numbered from `type="number"`, nested to where the parent item's content starts, with `<listheader>` as an unmarked line above the items. The structure comes from the tags alone and none of it from the source layout, so the indented multi-line form Visual Studio's snippet writes renders the same as a one-line one, and prose after `</list>` is not absorbed into the last item as a lazy continuation.
...
- `csMdDocs.defaultCodeLanguage`, defaulting to `csharp`, so a fence written without a language is syntax highlighted rather than grey. `<code language="xml">` and `<code lang="…">` label the fence with what the element says instead.
